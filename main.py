...
@app.route('/')
def home():
    return "Bot de Sinais Ativo (sinal forçado para teste)"
...
def calcula_sinal(_):
    print("[DEBUG] Forçando sinal COMPRA para teste")
    return "COMPRA"

def pegar_velas(ativo, API_KEY):
    return [{}]*20  # Mock para simular retorno válido
...
def monitorar_ativo(ativo, bot):
    ...
    while True:
        ...
        mensagem = (f"⏰ {agora}\n"
                    f"Ativo: <b>{ativo}</b>\n"
                    f"Sinal: <b>{sinal}</b>\n"
                    f"(Mensagem de teste - sinal forçado)")
