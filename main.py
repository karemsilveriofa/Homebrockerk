...
def calcula_sinal(velas):
    ...
    if len(closes) < 20:
        print("[DEBUG] Dados insuficientes para cálculo de sinal")
        return None

    ma_5 = sum(closes[-5:]) / 5
    ma_10 = sum(closes[-10:]) / 10

    print(f"[DEBUG] MA5: {ma_5:.5f} | MA10: {ma_10:.5f}")
...
def pegar_velas(ativo, API_KEY):
    ...
    try:
        ...
        data = resposta.json()
        if 'values' in data:
            print(f"[DEBUG] Velas recebidas para {ativo}: OK")
...
def monitorar_ativo(ativo, bot):
    ...
    while True:
        ...
        if sinal and sinal != ultimo_sinal:
            ...
            print(f"[DEBUG] Nenhum novo sinal para {ativo}")
